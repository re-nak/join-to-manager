...
static void sendTask(void* unused) {
   ...
   INT8U           i;
   INT8U           rc;
   INT8U                     chargeBuf[2+sizeof(dn_api_rsp_get_charge_t)];
   dn_api_rsp_get_charge_t*  charge;
   INT8U                     respLen;
   INT16U                    ftemp;
   ...
   while (1) { // this is a task, it executes forever
      ...
      charge = (dn_api_rsp_get_charge_t*)(&chargeBuf[0]);
      ftemp  = (charge->tempFrac)*100/256;
      
      dnm_ucli_printf(
          "\r\nQTotal=%d[mC]  UpTime=%d[sec] Temp=%d.%d[C.]\r\n",
          htonl(charge->qTotal),
          htonl(charge->upTime),
          charge->tempInt,
          ftemp
      ...
      //for (i=0;i<12;i++) {
      //   dnm_ucli_printf(" 0x%02x",chargeBuf[i]);
      //}
      //dnm_ucli_printf("\r\n");
      
      
      for (i=0;i<10;i++) {
         pkToSend->locSendTo.payload[i]     = chargeBuf[i+2];
      }
      ...
      if (rc==DN_API_RC_OK) {
          ...
          for (i=0;i<10;i++) {
              dnm_ucli_printf(" 0x%02x",pkToSend->locSendTo.payload[i]);
          }
          dnm_ucli_printf("\r\n");
      } else {
